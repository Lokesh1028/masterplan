...
export default function InvoiceDetailPage() {
  ...
  async function downloadPDF() {
    ...
    doc.setFontSize(20); doc.setFont('helvetica','bold'); doc.text(profile?.storeName||'Store',14,25)
    doc.setFontSize(9); doc.setFont('helvetica','normal'); doc.setTextColor(100)
    if(profile?.address) doc.text(profile.address,14,32); if(profile?.phone) doc.text('Phone: '+profile.phone,14,37)
    ...
    doc.setDrawColor(229,231,235); doc.line(14,48,pw-14,48)
    doc.setFontSize(9); doc.setTextColor(100); doc.text('BILL TO',14,56)
    doc.setFontSize(11); doc.setTextColor(0); doc.setFont('helvetica','bold'); doc.text(invoice.customer?.name||'-',14,62)
    doc.setFont('helvetica','normal'); doc.setFontSize(9); doc.setTextColor(100); let y=67
    if(invoice.customer?.address){doc.text(invoice.customer.address,14,y);y+=5}; if(invoice.customer?.phone){doc.text(invoice.customer.phone,14,y);y+=5}
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl border p-8 print:border-0">
        <div className="flex justify-between items-start mb-8"><div><h1 className="text-2xl font-bold text-gray-900">{profile?.storeName||'Store'}</h1><div className="text-sm text-gray-500 mt-1">{profile?.address&&<p>{profile.address}</p>}{profile?.phone&&<p>Phone: {profile.phone}</p>}</div></div><div className="text-right"><h2 className="text-3xl font-bold text-green-500">INVOICE</h2><p className="text-gray-600 font-medium mt-1">{invoice.invoiceNumber}</p><p className="text-sm text-gray-500">{formatDate(invoice.createdAt)}</p></div></div>
        <hr className="mb-6"/>
        <div className="mb-6"><p className="text-xs text-gray-400 uppercase tracking-wider mb-1">Bill To</p><p className="font-semibold text-gray-900">{invoice.customer?.name||'-'}</p>{invoice.customer?.address&&<p className="text-sm text-gray-500">{invoice.customer.address}</p>}{invoice.customer?.phone&&<p className="text-sm text-gray-500">{invoice.customer.phone}</p>}</div>
